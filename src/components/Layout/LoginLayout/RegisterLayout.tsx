...
const [alamat, setAlamat] = useState<string>("")
  ...
  useEffect(()=>{
    ...
    const getKonfirmasiPassword = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
        ...
        <div className=" flex flex-col justify-center items-center">
            ...
            className="w-[350px]" />
            <p className="font-bold text-2xl mt-7">Jual Beli Mudah Hanya di BeliBeli</p>
          ...
          show? (
           <div>
            <BannerDaftarlayout text="daftar sekarang"/>
             <Input name="nama" id="namauser" placeholder="masukan nama" textLabel="nama" type="text" textTransform="capitalize"
             getValue={getUserName} value={userName}
             />
             <Input name="email" id="emailuser" placeholder="contoh@mail.com" textLabel="email" type="email" textTransform="lowecase"
             getValue={getEmail} value={email}/>

             <Input name="nohp" id="nohpuser" placeholder="0821xxxxxxx" textLabel="no handphone" type="text"  textTransform="capitalize"
             getValue={getPhoneNumber} value={phoneNumber}/>

             <Input name="password" id="passworduser"  textLabel="pasword" type="password" 
             getValue={getPassword} value={password}/>

             <Input name="konfirmasipassword" id="konfirmasipassworduser"  textLabel="konfirmasi password" type="password" 
             getValue={getKonfirmasiPassword} value={konfirmasiPassword}/>
             <Button type="button" text="selanjutnya" getClick={getShow}/> 
             <MasukLayoutTyphography text1="Sudah punya akun BeliBeli?" text2="masuk"/>
             </div>) 
              :(
            <div>
             <BannerDaftarlayout text="detail alamat pengguna"/>
             <Input name="alamat" id="alamatuser" placeholder="masukan nama jalan, nomor dan patokan rumah " textLabel="alamat" type="text"  textTransform="capitalize"
             getValue={getAlamat} value={alamat}/>
...
const DaftarlayoutTyphography = () => {
  return (
    ...
    <p className="text-xs font-thin text-gray-700">Dengan mendaftar saya menyetujui dengan
    <span className="m-1 font-base text-green-600 capitalize"> syarat dan ketentuan</span>
    serta 
    <span className="m-1 font-base text-green-600 capitalize"> kebijakan privasi</span>
    </p>
    </div>
  )
}
...
const MasukLayoutTyphography = (props:{text1:string, text2:string}) => {
  return (
    ...
    <span className="ms-1 font-base text-green-600 capitalize">
      <Link to={"/login"}> {props.text2}</Link></span></span>
    </div>
  )
}
